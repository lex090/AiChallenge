...
import com.ai.challenge.context.StickyFactsContextManager
...
val appModule = module {
    ...
    single<ContextManager>(named("default")) {
        DefaultContextManager(strategy = get(), compressor = get(), summaryRepository = get())
    }
    single<ContextManager>(named("slidingWindow")) {
        SlidingWindowContextManager(windowSize = 10)
    }
    single<ContextManager>(named("stickyFacts")) {
        StickyFactsContextManager(factExtractor = get(), factRepository = get(), windowSize = 10)
    }
    single<ContextManager>(named("branching")) {
        BranchingContextManager(branchRepository = get(), windowSize = 10)
    }
    single {
        DelegatingContextManager(
            managers = mapOf(
                ContextStrategyType.SlidingWindow to get<ContextManager>(named("slidingWindow")),
                ContextStrategyType.StickyFacts to get<ContextManager>(named("stickyFacts")),
                ContextStrategyType.Branching to get<ContextManager>(named("branching")),
            ),
            initialStrategy = ContextStrategyType.SlidingWindow,
        )
    }
    single<Agent> {
        AiAgent(
